...
check "no existing config: plugins has exactly the 3 ruixen ids" \
  "$(jq -c '[.plugins[].id] | sort' <<<"$out1")" \
  '["ruixen.frame-widget","ruixen.notch","ruixen.settings"]'
...
check "customized: ruixen plugin ids present exactly once each (idempotent, not duplicated)" \
  ...
  '["ruixen.frame-widget","ruixen.notch","ruixen.settings","third-party.widget"]'
...
check "already ruixen.bar: existing ruixen plugin entry's extra field survives" \
  ...
  '{"id":"ruixen.notch","someFutureField":true}'
check "already ruixen.bar: missing ruixen id (frame-widget) still gets appended" \
